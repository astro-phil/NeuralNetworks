...
class Field(object):
    def __init__(self,size = (1500,1000),limit = 150):
        self.size = size
        self.limit = limit
...
class Boid(object):
    def __init__(self,idx = 0,screen = None,field = None,others = None,init_rot = 0,init_pos = (0,0),init_speed = 1,view = 100):
        ...
        if idx == 0:
            self.form = Triangle(screen,red,(20,10))
        else:
    ...
    def flocking(self,irotm):
        ...
        # cohesion
        self.speed += np.clip(aveboid[2]/self.view,-0.02,0.02) * np.sign(vector[0])
        self.rot -= np.clip(aveboid[2]/self.view * -np.sign((vector[0]+50)*vector[1]),-0.01,0.01)
        
        # separation
        if minboid[2] < 30:
            self.speed += np.clip((minboid[2]-30)/30*np.sign(minvector[0]),-0.1,0.1)
            self.rot += np.clip((minboid[2]-30)/30*np.sign(minvector[1]),-0.05,0.05) * np.sign(minvector[0]+50) 
...
class Flock(object):
    ...
    def create_swarm(self):
        ...
        for x in range(self.size):
            ...
            self.boids.append(Boid(x,self.screen,self.field,self.boids,init_rot,(init_x,init_y),init_speed,150))
